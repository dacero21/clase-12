...
pacman::p_load(tidyverse,sf,raster) # cargar y/o instalar paquetes a usar
...
manzana = st_read(dsn = 'data/input/MGN_URB_MANZANA.shp')
via = st_read(dsn = 'data/input/VIAS.shp')
toponimia = st_read(dsn = 'data/input/MGN_URB_TOPONIMIA.shp')

browseURL(url = "https://datosabiertos.bogota.gov.co/dataset/unidad-de-planeamiento-bogota-d-c", browser = getOption("browser")) # UPZ 
upz <- st_read(dsn = "data/input/UPZ.shp",stringsAsFactors = F) %>% 
...
ggplot() + geom_sf(data = crop_via,col='red',size=0.1) + 
           geom_sf(data = inter_via,col='blue',size=0.1) + geom_sf(data = upz[1,],fill=NA,col='black')
...
#### 4.2. Buffer
b_teusa = st_buffer(x = upz[1,] , dist = 2) # Ojo con el CRS
...
ggplot() + geom_sf(data = upz[1,],fill='red',col='black',alpha=0.1) + geom_sf(data = b_teusa,col='blue',fill='cyan',alpha=0.1)

#### 4.3. Dejemos solo la zona de influencia
buffer = st_difference(x = b_teusa,y = upz[1,])
ggplot() + geom_sf(data = upz[1,],fill='red',col='black',alpha=0.3) + geom_sf(data = buffer,col='blue',fill='cyan',alpha=0.3)
...
c_manzana = st_transform(x = c_manzana,crs=st_crs(upz))

ggplot() + geom_sf(data = upz,fill=NA,col='black',size=0.2) + geom_sf(data = c_manzana,size=0.1)
